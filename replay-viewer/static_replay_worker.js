// static_replay_worker.js — Worker half of the static replay core: owns the
...
function stageNote() {
  ...
  try {
    var length = Module._halite_stage_len ? Module._halite_stage_len() : 0;
    if (!length) return '';
    var pointer = Module._halite_stage_ptr();
...
function runtimeError() {
  var length = Module._halite_error_len();
  if (!length) {
    var stage = stageNote();
    return stage
  ...
  var pointer = Module._halite_error_ptr();
  return new TextDecoder().decode(
    Module.HEAPU8.slice(pointer, pointer + length));
}
...
function ingestPacket() {
  var length = Module._halite_packet_len();
  if (!length) throw new Error('Replay runtime produced an empty frame');
  var pointer = Module._halite_packet_ptr();
...
function sendRuntimeInput(bytes) {
  ...
  copyIntoRuntime(bytes, function (pointer, length) {
    Module._halite_input(pointer, length);
  });
}
...
async function start() {
  ...
  try {
    ...
    if (message.replayBytes) {
      // Adaptation (1): the page already fetched the document (it needs it for
      // the chrome), so the bytes arrive transferred instead of re-fetched.
    ...
    var loaded = copyIntoRuntime(bytes, function (pointer, length) {
      return Module._halite_load_replay(pointer, length);
    });
...
function applyInputNow() {
  ...
  try {
    if (Module._halite_frame() < 0) throw new Error(runtimeError());
    ingestPacket();
...
importScripts('./broadcast_core.js', './halite_replay.js');
