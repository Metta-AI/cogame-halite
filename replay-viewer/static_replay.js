// static_replay.js — page-side half of the static replay core: spawns the
// Worker, hands it the OffscreenCanvas + replay, and exposes the same view
...
// another's emscripten bootstrap deadlocks the viewer silently with every file
// present and every request 200 (cogame-lantern, 2026-08-23).
(function () {
  'use strict';
  ...
  function createCore(config) {
    ...
    function start(replayBytes) {
      if (started || !offscreen || failed) return;
      started = true;
      // Adaptation (1): the page fetches the replay itself and hands the bytes
      // over, so one fetch serves both the chrome and the renderer. The
      // ?replay= URL stays as the fallback for a bare bundle open.
      var replayUrl = new URLSearchParams(location.search).get('replay') || '/replay-data';
      if (!replayBytes && !replayUrl) {
      ...
      try {
        worker = new Worker(workerUrl, { name: 'halite-static-replay' });
        ...
        if (replayBytes) transfer.push(replayBytes);
        worker.postMessage({
          type: 'init',
          replayUrl: replayUrl,
          replayBytes: replayBytes || null,
          canvas: offscreen,
          width: viewport.width,
          height: viewport.height,
          dpr: viewport.dpr
    ...
    return {
      start: start,   // start(replayBytes?) — adaptation (1)
      stop: stop,
